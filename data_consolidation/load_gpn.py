...
class TGMSA(td.Dataset):
    ...
    def __getitem__(self, i):
        row = self.ds[i]; out = slice_window(row)
        while out is None:                        # rare gaps
            i = (i+1) % len(self.ds); row = self.ds[i]; out = slice_window(row)
        ref, alt, attn = out
        return dict(ref=torch.tensor(ref), alt=torch.tensor(alt),
                    attn=torch.tensor(attn), label=torch.tensor(row["label"]))
